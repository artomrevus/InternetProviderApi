...
[Route("[controller]")]
[ApiController]
public class InternetTariffController(IInternetTariffService service, IMapper<IInternetTariff, InternetTariffRequestDto, InternetTariffResponseDto> mapper) : ControllerBase
{
    ...
    public async Task<IActionResult> Add([FromBody] InternetTariffRequestDto dto)
    {
        try
        {
    ...
    [Route("{id:int:min(1)}")]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> Update([FromRoute] int id, [FromBody] InternetTariffRequestDto dto)
    {
        try
        {
