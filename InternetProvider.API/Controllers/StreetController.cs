...
public class StreetController(IStreetService appService) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> Add([FromBody] StreetRequestDto dto)
    {
        try
        {
    ...
    [Route("{id:int:min(1)}")]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> Update([FromRoute] int id, [FromBody] StreetRequestDto dto)
    {
        try
        {
