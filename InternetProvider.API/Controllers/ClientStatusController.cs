...
[Route("[controller]")]
[ApiController]
public class ClientStatusController(IClientStatusService service, IMapper<IClientStatus, ClientStatusRequestDto, ClientStatusResponseDto> mapper) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> Add([FromBody] ClientStatusRequestDto dto)
    {
        try
        {
    ...
    [Route("{id:int:min(1)}")]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> Update([FromRoute] int id, [FromBody] ClientStatusRequestDto dto)
    {
        try
        {
