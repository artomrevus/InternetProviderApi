...
[Route("[controller]")]
[ApiController]
public class LocationTypeController(ILocationTypeService service, IMapper<ILocationType, LocationTypeRequestDto, LocationTypeResponseDto> mapper) : ControllerBase
{
    ...
    [HttpPost]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> Add([FromBody] LocationTypeRequestDto dto)
    {
        try
        {
    ...
    [Route("{id:int:min(1)}")]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> Update([FromRoute] int id, [FromBody] LocationTypeRequestDto dto)
    {
        try
        {
